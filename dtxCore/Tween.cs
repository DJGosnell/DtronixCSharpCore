...
namespace dtxCore {
	...
	public class Tween {
		...
		/// <summary>
		/// Value to determine if tween action is complete.
		/// </summary>
		public bool is_working = false;

		/// <summary>
		///  If set to true, the second call to start will cause the animation to stop were it is and start a new animation.  False will cause the start method to do nothing.
		/// </summary>
		public bool interupt_tween = true;
		// TODO: add ability to chain tween actions. like in the docs here: http://mootools.net/docs/core/Fx/Fx
		...
		void timer_Tick(object sender, EventArgs e) {
			// Check to see if the time has already run out.
			if((time * timer.Interval) >= duration) {
				is_working = false;
				current = finish;
				timer.Stop();
			} else {
				// This seem to be incorrect, but I am not sure why.  The Tween seems to run longer than the total duration time.
			...
			// Step the animation on to the next frame.
			time++;
		}

		public void start(int from, int to, TweenDelegate tween_callback) {
			if(!interupt_tween && is_working) return;
			is_working = true;
